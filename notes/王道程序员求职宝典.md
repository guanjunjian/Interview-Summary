...
![](G:\OneDrive\Github\Interview-Summary\pics\WangDao\非虚继承_一次继承_内存分布图.png)
...
![](G:\OneDrive\Github\Interview-Summary\pics\WangDao\虚继承_一次继承_内存分布图.png)
...
- 3.const_cast：将const属性去掉（不会出错的情况：指向const变量的指针，指针指向的对象不是const的）
- 4.static_cast：类型之间的转换，以下情况是合法的：
  - 类型之间可以隐式转换时
  - 基类指针或引用转换为派生类指针或引用，但由于没有动态检查，所以不安全
- 5.dynamic_cast，
  - 以下情况是合法的
    ...
    - type为`void*`
  - 出错情况
    - dynamic_cast涉及运行时类型检查，因此没有虚函数表的类使用它会编译时出错
    - dynamic_cast转换到指针时失败，返回值为0
    - dynamic_cast转换为引用时失败，抛出`bad_cast`异常
  - 执行步骤：
    - 验证被请求的转换是否有效（在运行时检查）
    - 有效时，才执行转换
  - 主要用途：
    - 下行转换（基类到派生类）：比static_cast安全，因为有类型检查

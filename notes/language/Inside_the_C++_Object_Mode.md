...
- 方式二：
  - 不需要临时性对象，也不需要对临时性对象构造函数和析构函数进行调用
...
情况2不能将c直接传递给`operator+`运算符操作，因为`operator+`并不会调用c对象的析构函数，而`operator=`会先调用析构函数，后调用拷贝构造函数。我们是希望将临时性对象赋值给一块“新鲜”的内存
...
![](../../pics/language/Inside_the_C++_Object_Model/typeid%E7%A4%BA%E8%8C%83.png)

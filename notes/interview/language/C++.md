...
MyStruct A, B;
memcmp(&A, &B, sizeof(MyStruct)); //不能进行正确的笔记
```
...
二进制浮点数是以符号数值表示法的格式存储——浮点数由三部分组成：符号（sign部分，表示符号0正，1负数），指数（exponent部分，表示指数位），和尾数（fraction部分，表示有效数字，大于等于1，小于2）。 

十进制的5.0，写成二进制是101.0，相当于1.01×2^2。那么，按照上面那段话，可以得出符号位sign为0，fraction部分为1.01，exponent部分为2。 
